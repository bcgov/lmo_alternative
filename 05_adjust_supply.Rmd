...
multiple_98100446 <- list.files(here("data", "data_parquet"), pattern = "98100446", full.names = TRUE)
most_recent_98100446 <- multiple_98100446[which.max(file.info(multiple_98100446)$mtime)]

immigrant_part_rate <- open_dataset(most_recent_98100446)|>
...
multiple_98100593 <- list.files(here("data", "data_parquet"), pattern = "98100593", full.names = TRUE)
most_recent_98100593 <- multiple_98100593[which.max(file.info(multiple_98100593)$mtime)]

entrant_noc_props <- open_dataset(most_recent_98100593)|>
  dplyr::filter(geo == "British Columbia",
                statistics_3=="Count",
...
multiple_98100449 <- list.files(here("data", "data_parquet"), pattern = "98100449", full.names = TRUE)
most_recent_98100449 <- multiple_98100449[which.max(file.info(multiple_98100449)$mtime)]

canada_noc_props <- open_dataset(most_recent_98100449)|>
  filter(geo == "Canada",
  ...
  mutate(count=as.numeric(count),
    ...
    arrange(desc(prop))
############################################################
multiple_98100316 <- list.files(here("data", "data_parquet"), pattern = "98100316", full.names = TRUE)
most_recent_98100316 <- multiple_98100316[which.max(file.info(multiple_98100316)$mtime)]

immigrant_noc_props <- open_dataset(most_recent_98100316)|>
  filter(geo=="Canada",
...
output$download_full <- downloadHandler(
  filename = function() {
    paste0("supply_and_demand_", Sys.Date(), ".csv")
  },
