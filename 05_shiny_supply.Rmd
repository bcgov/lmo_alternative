...
bc_part_rate <- bc_part_rate_historic|>
  ...
  forecast(h=forecast_horizon+1)|> #data ends in 2024 so need to extend forecast horizon
  as_tibble()|>
  group_by(ref_date)|>
  summarize(part_rate=mean(.mean))|>
  filter(ref_date %in% forecast_years) #discard 2025
...
stokes_total <- read_lengthen_clean("supply_demand.xlsx")|>
  filter(noc=="#T",
...
For net inter-provincial migrants, we use data from Statistics Canada table [17-10-0015-01](https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=1710001501).  For the participation rate for inter-provincial migrants, we use Statistics Canada table [14-10-0327-01](https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=1410032701) and filter for Canada. For both these series we create an ensemble forecast using ETS and TSLM, which is plotted in blue.
...
output$results <- renderReactable({
  joined_reactive()|>
     reactable(columns = list(rich = colDef(format = colFormat(digits = 0, separators = TRUE)),
                              stokes=colDef(format = colFormat(digits = 0, separators = TRUE)) 
                              ),
               pagination = FALSE,
               compact = TRUE,
               bordered = TRUE,
