...
find_corr_outliers <- function(tbbl, rich, stokes, n) {#note that ties are dropped.
  fmla <- reformulate(deparse(substitute(stokes)), deparse(substitute(rich)))
  fit <- lm(fmla, data = tbbl)
  cooks <- cooks.distance(fit)
  distance <- log(cooks[order(cooks, decreasing = TRUE)])
  bind_cols(tbbl[order(cooks, decreasing = TRUE), ], log_distance=distance)
}
...
jo_region_industry|>
  find_corr_outliers(richs_expansion, stokes_expansion)|>
...
jo_region_industry|>
  find_corr_outliers(richs_replacement, stokes_replacement)|>
