...
rich_vs_stokes <- function(tbbl, var1, var2, cor){
  plt <- ggplot(tbbl,  aes({{  var1  }},
    ...
    geom_point(mapping=aes(fill=year,
                ...
                shape = 21, size=1.5, colour = "black", stroke=.25)+
...
fast_loocor <- function(x, y) {
  ...
  for (i in seq_len(n)) {
    ...
    # Sums of squares (centered)
    Sxx <- sx2 - nx * mx^2
    Syy <- sy2 - nx * my^2
    Sxy <- sxy - nx * mx * my
    
    cors[i] <- Sxy / sqrt(Sxx * Syy)
  }
  cors
}


loocor_df <- function(data, xcol, ycol) {
  # allow bare column names
  xcol <- ensym(xcol)
  ycol <- ensym(ycol)
  
  x <- data[[as_string(xcol)]]
  y <- data[[as_string(ycol)]]
  
  # full correlation
  r_full <- cor(x, y)
  
  # fast leave-one-out correlations
  cors <- fast_loocor(x, y)  
  delta <- cors - r_full
  
  # return original df with delta
  data %>%
    mutate(r_full=r_full,
           delta = delta,
           cor_without = cors)|>
    arrange(desc(delta))
}
...
demand_plot <- function(tbbl, rich, stokes, title){
  ...
  geom_point(shape = 21, size=1.5, colour = "black", stroke=.25)+
  scale_fill_viridis_c()+
...
Demand: Coloured by Leave One Out $\Delta$ correlation
=====================================
...
jo_region_industry|>
  ungroup()|>
  loocor_df(richs_expansion, stokes_expansion)|>
  demand_plot(richs_expansion, stokes_expansion, title)
```
...
jo_region_industry|>
  ungroup()|>
  loocor_df(richs_replacement, stokes_replacement)|>
  demand_plot(richs_replacement, stokes_replacement, title)
```
...
jo_occupation|>
  ungroup()|>
  loocor_df(richs_expansion, stokes_expansion)|>#view()
  demand_plot2(richs_expansion, stokes_expansion, title)
```
...
jo_occupation|>
  ungroup()|>
  loocor_df(richs_replacement, stokes_replacement)|>
  demand_plot2(richs_replacement, stokes_replacement, title)
```
