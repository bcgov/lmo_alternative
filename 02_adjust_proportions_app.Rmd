...
-   Specifically, we create a weighted average of Census and LFS proportions, where the weight on Census is 0.81 and the weight on LFS is 0.19.
...
kable(
  tibble(
    ...
    `Effective N (workers)` = c("~1.1 million", "~4.8 million")
  ),
...
-   We then aggregate the LFS data from 2000 to 2024, creating 582 slices of BC (7 regions, 63 industries and 512 occupations), to each of which we apply the linear model:

$$ log(Employment+1) \sim time+\epsilon$$

-   The smaller the slice the more noisy it is.
-   Trust in the regression estimates should depend on how noisy the series is.  
-   Empirical Bayes is a method that shrinks the slope estimates towards the slope of BC as a whole.
-   The degree of shrinkage depends on the variance of the slope estimate (standard error squared), relative to the variance of the slope deviations (difference between each slice's slope estimate and BC's slope estimate).  
-   We then apply these shrunken growth factors to adjust the baseline proportions over the forecast horizon.
