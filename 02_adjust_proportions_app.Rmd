...
runtime: shiny
resource_files:
- out/modified_shares/shares.rds
- out/bc_forecast.rds
...
*  We then aggregate the LFS data from 2000 to 2023, creating 583 series (7 regions, 64 industries and 512 occupations) to which we apply the linear model: 
...
*  If p.value > .05 there is little evidence that the series' growth rate differs from BC as a whole.
*  We create a weighted average of the growth factors, where the weight on BC's growth factor is $p.value^.02$, and the weight on "this series" growth factor is $1-p.value^.02$. The relationship is shown $\rightarrow$
*  Note that this is a pretty aggresive shrinkage towards BC's overall growth factor. 
...
### Relationship between p values and weight on BC's growth rate

```{r, fig.retina=2}
options(scipen = 999)
tibble(p.value=.5^(0:50))|>
  mutate(bc_weight=p.value^.02)|>
  ggplot(aes(bc_weight, p.value))+
  geom_line()+
  scale_y_continuous(trans="log10", breaks =.1^(seq(0,20,2)))+
  scale_x_continuous(trans="log10", breaks = seq(0,1,.1))
```
