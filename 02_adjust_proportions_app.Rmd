...
* At a given point in time, one can visualize the remaining 3 dimensions as a cube, with sides representing the 7 regions, 64 industries, and 512 occupations, and cells representing the proportion of BC employment for that combination of region, industry and occupation.
* Note that this cube is extremely sparse: there are not many combinations of region, industry and occupation that have positive counts.
...
*  Using LFS data from the years 2021,2022,2023 we calculate baseline proportions of BC employment based on every combination of region, industry and occupation.
*  We then aggregate the data by each of the above margins, smooth the data via a 3 year moving average, and then calculate the median annual growth rate.
*  Note that if smoothed employment is ever zero and then subsequently increases, the annual growth rate is $\infty$.
*  We use robust measures such as the median and inter-quartile range to deal with this possibility.
*  We also shrink the individual growth rates towards the overall (BC) median growth rate via a weighted average.
*  The weights we utilize depend the relative spread $rs$ of the annual growth: 

$$rs=IQR(annual~growth)/median(annual~growth)$$

*  The weight we put on B.C.'s median annual growth rate is $\frac{rs}{rs+rs_{BC}}$
*  i.e. the greater the relative spread in the annual growth rates the less we trust the data, so we put a greater weight on B.C.'s median annual growth rate.
*  We then apply these marginal growth rates to adjust the baseline proportions.
*  For example, suppose that a region/industry/occupation combination accounted for 1% of the BC labour market over the years 2021,2022,2023.  Suppose that region's growth factor is 1.02, the industry's growth factor is 1.03, and the occupation's growth factor is .96
