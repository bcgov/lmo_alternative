...
- Forecast would be available within 10 minutes of budget numbers being released.
...
-  The model currently only considers employment levels and demand. (no supply model) 

#### Correlation between Stokes and Rich:
...
- Expansion Demand:
    -  When aggregated to industry:region combos (63*7=441) the correlation is $\approx$ .87
    -  When aggregated to occupation (512) the correlation is $\approx$ .8
...
-   Specifically, the baseline proportions are a weighted average of Census and LFS proportions, where the weight on Census is 0.83 and the weight on LFS is 0.17.
...
kable(
  tibble(
    ...
    `Effective N (distinct workers)` = c("~1 million", "~4.8 million")
  ),
...
-   Distinct households sampled over 84 months ≈ (56,000/6)*84 = 784,000.
-   Given the 1.28 workers per household calculated above, LFS covered approximately 1 million distinct workers over 7 years.
...
-   Obviously the composition of the BC labour market is not fixed: Next we describe how we model the evolution of the proportions over time.
-  We first estimate the growth rate of BC as a whole by fitting this model:

$$ log(Employment+1) \sim year+\epsilon$$

-  The slope coefficient from this regression $\hat{\beta}_{BC}$ will serve as our prior belief: what we would expect in the absence of any other information. 
-  We slice BC into 7 regions, 63 industries and 512 occupations: 582 slices of BC's labour market.
-  We fit the above model to each slice, yielding 582 slope coefficients $\hat{\beta_s}$, and their standard errors $SE(\hat{\beta}_s)$.
-  The question is how convincing is each slice's slope estimate? i.e. How much should we update our prior belief?
-  Two bits of information are relevant to belief updating: 
    1) $SE(\hat{\beta}_s)$ indicates the precision of the estimate, aka the amount of noise around the slice's trend. 
    2) $\tau^2=var(\hat{\beta}_s-\hat{\beta}_{BC})$ is the variance of the slope deviations from the prior: an indication of how much variation there is *between* the slices in a given dimension (e.g. Regions)
-  If a slice's slope is estimated precisely and there is a lot of variability between slices then we basically ignore the prior (yellow areas in heatmap to right).
-  If a slice's slope is estimated imprecisely and there is not much variability between slices then we weight the prior heavily (purple areas in heatmap to right).
-  Specifically, we create a weighted average of the prior and the slice's slope estimate, where the weight on $\hat{\beta}_s$ is
