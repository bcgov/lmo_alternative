...
-   We then aggregate the LFS data from 2000 to 2024, creating 583 series (BC as a whole, 7 regions, 63 industries and 512 occupations) to each of which we apply the linear model:

$$ log(Employment+1) \sim time+\epsilon$$

-   Note that the smaller the slice of data the more noisy the series.
-   The extent to which we should trust the regression estimates should depend on the relative noisiness of the series. 
-   Empirical Bayes is a method that shrinks the slope estimates towards BC's slope.
...
-   We then apply the shrunken growth factors to adjust the baseline proportions.
