...
bc_fcast <- read_rds(here("out","modified","bc_forecast.rds")) #the BC finance forecast plus ETS forecast for last 6 years.
...
bc_reactive <- reactive({
  bc_fcast|>
    mutate(employment=if_else(series=="our forecast", 
                              employment*input$bc_slope^(year-2029), 
                              employment))
})
...
-   Using LFS data from the years 2022:2024 we calculate baseline proportions of BC employment based on every combination of region, industry and occupation.
...
-   We create a weighted average of the growth factors, where the weight on BC's growth factor is $p.value^.02$, and the weight on "this series" growth factor is $1-p.value^.02$.
-   Note that this is a pretty aggressive shrinkage towards BC's overall growth factor. e.g. if p.value=.00001 (we are really quite confident that this series is growing at a different rate than BC as a whole), the weight on BC's growth factor is still .8.
...
-  Every cell on level 2 of the cube would increase by the same proportion.
-  Every cell not on level 2 would diminish by a different (but uniform) proportion to compensate.
